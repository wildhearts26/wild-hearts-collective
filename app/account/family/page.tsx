...
export default async function AccountFamilyPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-8 px-6 py-10">
          ...
          {member.household.map((person) => (
            <li key={person.id} className="flex flex-wrap items-center justify-between gap-3 py-4">
              <div>
                ...
                <p className="text-sm text-muted">{memberTypeLabel(person.memberType)}</p>
              </div>
              {person.isActive ? (
                <Link
                  href={BOOKING_URL}
      ...
      {member.isChild && (!member.parentalConsentComplete || !member.idDocumentUploaded) ? (
        <section className="rounded-sm border border-brand/20 bg-pink-soft/50 p-6">
          <h3 className="font-display text-2xl text-plum">Consent needed</h3>
          <p className="mt-2 text-sm text-muted">
            This child cannot book until parental consent and proof of identification are on file.
            Add or complete those details from the parent account if they are missing.
          </p>
        </section>
      ) : null}
